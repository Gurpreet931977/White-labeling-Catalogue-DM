      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-5 lg:px-6 xl:px-8">
        <div className="flex items-center justify-between h-16 sm:h-20 gap-2 lg:gap-4">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 lg:gap-2.5 shrink-0">
            ...
            {/* VIP Table Concierge Trigger (Seated Dine-In) */}
            {activeTable && onOpenConcierge && (
              <button
                onClick={() => { sounds.playClick(); onOpenConcierge(); }}
                className={`hidden md:flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-xl border text-xs font-mono transition cursor-pointer shrink-0 ${
                  isLight
                    ? 'bg-[#EAE4D9] border-black/10 text-[#12100E] hover:border-[#C5A880]'
                    : 'bg-[#1A1715] border-[#C5A880]/30 text-[#C5A880] hover:border-[#C5A880]'
                }`}
                title="Summon Table Concierge (Water, Silverware, Floor Captain)"
              >
                <BellRing className="w-3.5 h-3.5 text-[#C5A880] animate-pulse" />
                <span className="hidden lg:inline text-[11px] font-bold">Concierge</span>
              </button>
            )}
            ...
            <div className="fixed inset-0 z-[100] lg:hidden flex justify-end">
              ...
              <div className={`p-5 sm:p-6 border-t space-y-3 font-mono text-xs ${
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  >
                    {isLight ? <Moon className="w-4 h-4 text-[#12100E]" /> : <Sun className="w-4 h-4 text-[#C5A880]" />}
