  ...
  return (
    <section className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-3 pb-1">
        ...
        <p className="text-xs text-[#76675B] font-mono">
          Unlocked perks can be presented to cashier for immediate redemption
        </p>
      </div>

      {/* 1. DAILY MYSTERY PERK (Interactive Discovery Parcel) */}
      <div className="p-6 sm:p-7 rounded-3xl bg-white border border-[#E2D6C3] relative overflow-hidden shadow-md">
        <div className="absolute top-0 right-0 w-64 h-64 bg-[#FF4800]/5 rounded-full blur-[80px] pointer-events-none" />
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-6 relative z-10">
          <div className="flex items-center gap-5 text-center sm:text-left flex-col sm:flex-row">
            <motion.div
              ...
              className="w-16 h-16 rounded-2xl bg-[#FFF5F0] border-2 border-[#FF4800]/40 flex items-center justify-center cursor-pointer shadow-md hover:scale-105 transition-transform shrink-0"
            >
              {mysteryRevealed ? (
                <Sparkles className="w-8 h-8 text-[#FF4800]" />
              ) : (
                <Gift className="w-8 h-8 text-[#FF4800]" />
              )}
            </motion.div>
            ...
            <div className="space-y-1">
              <div className="flex items-center gap-2 justify-center sm:justify-start">
                ...
                <span className="px-2 py-0.5 rounded bg-[#EAE0CE] text-[#6E5D4F] font-mono text-[9px] font-bold">
                  RESETS DAILY
                </span>
              </div>
              ...
              <p className="text-xs text-[#76675B] max-w-md">
                {mysteryRevealed
                  ? mysteryPerk?.desc
          ...
          <div className="shrink-0">
            {mysteryRevealed ? (
              <div className="px-5 py-2.5 rounded-2xl bg-[#10B981]/15 border border-[#10B981]/30 text-[#047857] font-mono text-xs font-bold flex items-center gap-2">
              ...
              <button
                type="button"
                onClick={handleShakeMystery}
                className="px-6 py-3 rounded-2xl bg-[#FF4800] hover:bg-[#E03F00] text-white font-sans font-bold text-xs tracking-wide shadow-md shadow-[#FF4800]/25 cursor-pointer transition-all flex items-center gap-2"
              >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {tieredRewards.map((perk) => {
          ...
          return (
            <div
              key={perk.id}
              className={`relative rounded-3xl p-6 border flex flex-col justify-between transition-all overflow-hidden ${
                perk.isGrand
                  ? isUnlocked
                    ? 'bg-[#FFF9F6] border-2 border-[#FF4800] shadow-lg ring-2 ring-[#FF4800]/20'
                    : 'bg-white border-[#E2D6C3]'
                  : isUnlocked
                  ? 'bg-white border-2 border-[#1C120C] shadow-md'
                  : 'bg-[#FBF8F2] border-[#E2D6C3] opacity-80'
              }`}
            >
              {/* Perforation notches on left and right edges */}
              <div className="absolute -left-3 top-1/2 -translate-y-1/2 w-6 h-6 rounded-full bg-[#F7F2E7] border border-[#E2D6C3] pointer-events-none" />
              <div className="absolute -right-3 top-1/2 -translate-y-1/2 w-6 h-6 rounded-full bg-[#F7F2E7] border border-[#E2D6C3] pointer-events-none" />

              <div>
                {/* Header Row */}
                <div className="flex items-center justify-between gap-2 pb-3.5 border-b border-[#E2D6C3]">
                  <div className="flex items-center gap-1.5">
                    <span className={`text-[10px] font-mono font-bold uppercase tracking-wider ${
                      perk.isGrand ? 'text-[#FF4800]' : isUnlocked ? 'text-[#1C120C]' : 'text-[#8C7D70]'
                    }`}>
                      {perk.badge}
                    </span>
                    {perk.isGrand && (
                ...
                <div className="pt-4 space-y-1.5">
                  <h4 className="font-clash font-bold text-lg text-[#1C120C] tracking-tight">
                    {perk.title}
                  </h4>
                  <p className="text-xs text-[#76675B] font-sans line-clamp-2">
                    {perk.desc}
                  </p>
                </div>
              </div>
              ...
              {/* Action Bottom Row */}
              <div className="pt-6 mt-4 border-t border-dashed border-[#E2D6C3] flex items-center justify-between">
                {isUnlocked ? (
                  <>
                    <span className="text-[10px] font-mono text-[#047857] font-bold flex items-center gap-1">
                    ...
                    <button
                      ...
                      className="px-4 py-2 rounded-xl text-xs font-bold cursor-pointer transition-all flex items-center gap-1.5 bg-[#FF4800] hover:bg-[#E03F00] text-white shadow-sm shadow-[#FF4800]/25"
                    >
                      <span>Redeem</span>
                  ...
                  <>
                    <span className="text-[10px] font-mono text-[#8C7D70] flex items-center gap-1">
                      <Lock className="w-3.5 h-3.5" />
                      <span>{stampsNeeded} more stamps</span>
                    </span>
                    <span className="text-[10px] font-mono text-[#8C7D70] uppercase font-bold">
                      LOCKED
                    </span>
                  </>
                )}
        ...
        {activeVoucher && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#1C120C]/80 backdrop-blur-md">
            <motion.div
              ...
              className="relative w-full max-w-md rounded-3xl bg-white border border-[#E2D6C3] shadow-2xl p-6 sm:p-8 space-y-6 overflow-hidden"
            >
              {/* Close Button */}
              <button
                type="button"
              ...
              <div className="p-4 rounded-2xl bg-[#FFF9F6] border border-[#FF4800]/30 text-center space-y-1">
                <div className="flex items-center justify-center gap-2 text-xs font-mono text-[#FF4800] font-bold">
                  <Clock className="w-4 h-4" />
                  <span>VOUCHER ACTIVE WINDOW</span>
                </div>
                <div className="font-mono text-3xl font-black text-[#1C120C] tracking-wider">
                  {formatTimer(timeLeft)}
                </div>
                <p className="text-[10px] font-mono text-[#76675B]">
                  Pass refreshes automatically to prevent unauthorized replication
                </p>
              </div>

              {/* Barcode & Code Display */}
              <div className="p-5 rounded-2xl bg-[#F8F4EC] border border-[#E2D6C3] text-center space-y-3">
                ...
                <div className="h-14 flex items-center justify-center gap-[3px] px-4 overflow-hidden">
                  {[2, 4, 1, 3, 2, 5, 1, 2, 4, 3, 1, 4, 2, 3, 5, 2, 1, 3, 4, 1, 2, 5, 3, 2, 4, 1, 3, 2, 4].map((h, idx) => (
                    <div
                      key={idx}
                      className="bg-[#1C120C] rounded-xs"
                      style={{
                        width: `${h}px`,
                        height: '100%',
                ...
                <div className="flex items-center justify-center gap-2">
                  <span className="font-mono text-sm font-bold text-[#1C120C] tracking-wider">
                    {activeVoucher.code}
                  </span>
                  <button
                    type="button"
                    onClick={() => handleCopyCode(activeVoucher.code)}
                    className="p-1.5 rounded-lg bg-white border border-[#E2D6C3] text-[#76675B] hover:text-[#1C120C] cursor-pointer"
                  >
              ...
              <button
                ...
                className="w-full py-3.5 rounded-2xl bg-[#1C120C] hover:bg-black text-white font-sans font-bold text-xs tracking-wider uppercase cursor-pointer shadow-lg transition-all flex items-center justify-center gap-2"
              >
